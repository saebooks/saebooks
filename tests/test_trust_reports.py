...
@pytest.mark.anyio
async def test_trust_cashbook_route_200(client: AsyncClient) -> None:
    resp = await client.get("/reports/trust-cashbook")
    assert resp.status_code == 200


@pytest.mark.anyio
async def test_trust_balances_route_200(client: AsyncClient) -> None:
    resp = await client.get("/reports/trust-balances")
    assert resp.status_code == 200


@pytest.mark.anyio
async def test_reports_index_contains_trust_links(client: AsyncClient) -> None:
    resp = await client.get("/reports")
    assert resp.status_code == 200
    assert "trust-cashbook" in resp.text
    assert "trust-balances" in resp.text
