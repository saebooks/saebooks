...
@pytest.mark.asyncio
async def test_tpar_report_page_returns_200(client: AsyncClient) -> None:
    r = await client.get("/reports/tpar")
    assert r.status_code == 200
    assert "TPAR" in r.text or "Taxable Payments" in r.text


@pytest.mark.asyncio
async def test_tpar_report_with_date_range(client: AsyncClient) -> None:
    r = await client.get("/reports/tpar?from=2025-07-01&to=2026-06-30")
    assert r.status_code == 200


@pytest.mark.asyncio
async def test_reports_index_links_to_tpar(client: AsyncClient) -> None:
    r = await client.get("/reports")
    assert r.status_code == 200
    assert "/reports/tpar" in r.text
    assert "TPAR" in r.text
