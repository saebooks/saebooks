...
def test_router_html_renders_200() -> None:
    client = _client()
    r = client.get("/reports/depreciation-schedule?as_at=2026-06-30")
    assert r.status_code == 200
    assert "Depreciation schedule" in r.text
    # Explanation fragment is always visible when there are rows, but it
    # may collapse to the empty-state paragraph if the DB has zero
    # active assets. Either way the page title must be present.
    assert "book" in r.text.lower()
    assert "tax" in r.text.lower()


def test_router_csv_download() -> None:
    client = _client()
    r = client.get(
        "/reports/depreciation-schedule?as_at=2026-06-30&format=csv"
    )
    assert r.status_code == 200
    assert r.headers["content-type"].startswith("text/csv")
    assert (
        'attachment; filename="depreciation-schedule-2026-06-30.csv"'
        in r.headers["content-disposition"]
    )
    # CSV header is always the first line regardless of row count
    assert r.text.splitlines()[0].startswith(
        "code,name,status,cost,book_model_id,"
    )


def test_reports_index_links_to_depreciation_schedule() -> None:
    client = _client()
    r = client.get("/reports")
    assert r.status_code == 200
    assert "/reports/depreciation-schedule" in r.text
    assert "Depreciation schedule" in r.text
