...
async def test_ytd_turnover_below_threshold(
    ...
    # Use the current FY's start date: always in the past once the FY has
    # begun (a fixed month like Aug/Sep is FUTURE-dated during Jul-Aug and
    # silently excluded from year-to-DATE), and always after the seeded
    # period lock 2026-03-31.
    today = date.today()
    fy_year = today.year if today.month >= 7 else today.year - 1
    entry_date = date(fy_year, 7, 1).isoformat()
...
async def test_ytd_turnover_above_threshold(
    ...
    # test_ytd_turnover_below_threshold.
    today = date.today()
    fy_year = today.year if today.month >= 7 else today.year - 1
    entry_date = date(fy_year, 7, 1).isoformat()
