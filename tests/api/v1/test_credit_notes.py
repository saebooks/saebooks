...
# ---------------------------------------------------------------------------
# PATCH on POSTED — non-financial allowlist (mirrors invoices / bills)
# ---------------------------------------------------------------------------


async def test_cn_patch_posted_financial_returns_422(
    api_client: AsyncClient, cn_deps: dict[str, str]
) -> None:
    """lines / issue_date on a POSTED credit note must 422 credit_note_not_draft."""
    r = await api_client.post("/api/v1/credit_notes", json=_cn_payload(cn_deps))
    assert r.status_code == 201, r.text
    cn_id = r.json()["id"]
    v = r.json()["version"]
    ...
    r2 = await api_client.post(
        f"/api/v1/credit_notes/{cn_id}/post", headers={"If-Match": str(v)}
    )
    assert r2.status_code == 200, r2.text
    v2 = r2.json()["version"]
    posted_total = r2.json()["total"]

    r3 = await api_client.patch(
        f"/api/v1/credit_notes/{cn_id}",
        json={"lines": _cn_payload(cn_deps)["lines"]},
        headers={"If-Match": str(v2)},
    )
    assert r3.status_code == 422, r3.text
    assert "credit_note_not_draft" in r3.text

    r4 = await api_client.patch(
        f"/api/v1/credit_notes/{cn_id}",
        json={"issue_date": "2026-01-31"},
        headers={"If-Match": str(v2)},
    )
    assert r4.status_code == 422, r4.text
    assert "credit_note_not_draft" in r4.text

    # Document untouched: version and totals as posted.
    r5 = await api_client.get(f"/api/v1/credit_notes/{cn_id}")
    assert r5.json()["version"] == v2
    assert r5.json()["total"] == posted_total


async def test_cn_patch_posted_nonfinancial_allowed(
    api_client: AsyncClient, cn_deps: dict[str, str]
) -> None:
    """notes / reason / payment_terms stay amendable on a POSTED credit note."""
    r = await api_client.post("/api/v1/credit_notes", json=_cn_payload(cn_deps))
    assert r.status_code == 201, r.text
    cn_id = r.json()["id"]
    v = r.json()["version"]
    ...
    r3 = await api_client.patch(
        f"/api/v1/credit_notes/{cn_id}",
        json={
            "notes": "corrected note after posting",
            "reason": "Goods returned — damaged in transit",
            "payment_terms": "Credit applied to next statement",
        },
        headers={"If-Match": str(v2)},
    )
    assert r3.status_code == 200, r3.text
    body = r3.json()
    assert body["version"] == v2 + 1
    assert body["notes"] == "corrected note after posting"
    assert body["payment_terms"] == "Credit applied to next statement"
    assert body["total"] == posted_total
    assert body["status"] == "POSTED"
