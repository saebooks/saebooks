...
# ---------------------------------------------------------------------------
# Document Inbox phase 0 (issue #33) — "expense" whitelist entry
# ---------------------------------------------------------------------------
# Publish attaches the source document to the DRAFT expense it becomes,
# so "expense" joins _ENTITY_MODELS. These regression tests pin (a) the
# happy path through the same upload→link facade and (b) that the
# tenant-ownership existence check holds for Expense (403, not a leak).


@pytest.fixture
async def seed_expense() -> uuid.UUID:
    """Create a fresh DRAFT Expense in the default tenant, return its UUID."""
    from datetime import date as _date
    from decimal import Decimal
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert payment_account is not None and contact is not None
...
async def test_upload_happy_path_expense(
    api_client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
    seed_expense: uuid.UUID,
) -> None:
    file_id = uuid.uuid4()
    captured: dict[str, Any] = {}

    async def fake_upload(tenant_id, *, file, filename, content_type, actor):
        captured["upload"] = {"tenant_id": tenant_id}
        return _file_meta(
            file_id, filename=filename, mime=content_type, size_bytes=len(file)
        )
    ...
    async def fake_link(tenant_id, fid, *, entity_kind, entity_id, actor):
        ...
        return {"id": str(uuid.uuid4()), "file_id": str(fid),
                "entity_kind": entity_kind, "entity_id": str(entity_id),
                "linked_at": "2026-07-04T01:00:00Z"}
    ...
    r = await api_client.post(
        "/api/v1/attachments",
        data={"entity_kind": "expense", "entity_id": str(seed_expense)},
        files={"file": ("receipt.jpg", b"JPG1", "image/jpeg")},
    )
    assert r.status_code == 201, r.text
    assert captured["upload"]["tenant_id"] == DEFAULT_TENANT_ID
    assert captured["link"]["entity_kind"] == "expense"
    assert captured["link"]["entity_id"] == seed_expense


async def test_upload_cross_tenant_expense_returns_403(
    api_client: AsyncClient,
) -> None:
    """An expense id the tenant doesn't own is rejected 403 by the
    ownership SELECT — without leaking whether the row exists."""
    bogus = uuid.uuid4()
    r = await api_client.post(
        "/api/v1/attachments",
        data={"entity_kind": "expense", "entity_id": str(bogus)},
        files={"file": ("receipt.jpg", b"JPG1", "image/jpeg")},
    )
    assert r.status_code == 403


async def test_list_expense_kind_accepted(
    api_client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
    seed_expense: uuid.UUID,
) -> None:
    """GET with entity_kind=expense passes _validate_kind and forwards
    the filter to the vault list call."""
    captured: dict[str, Any] = {}

    async def fake_list(tenant_id, *, entity_kind=None, entity_id=None,
                        include_archived=False):
        captured["args"] = (entity_kind, entity_id)
        return []
    ...
    r = await api_client.get(
        "/api/v1/attachments",
        params={"entity_kind": "expense", "entity_id": str(seed_expense)},
    )
    assert r.status_code == 200, r.text
    assert captured["args"] == ("expense", seed_expense)


# ---------------------------------------------------------------------------
# Document Inbox phase 2 (issue #33) — "credit_note" whitelist entry
# ---------------------------------------------------------------------------
# CREDIT_NOTE joined the publish kinds, so "credit_note" joins
# _ENTITY_MODELS. Same regression pins as the phase-0 expense entry:
# (a) happy path through the upload→link facade and (b) the
# tenant-ownership existence check holds for CreditNote (403, no leak).


@pytest.fixture
async def seed_credit_note() -> uuid.UUID:
    """Create a fresh DRAFT CreditNote in the default tenant, return its UUID."""
    from datetime import date as _date
    from decimal import Decimal
...
async def test_upload_happy_path_credit_note(
    api_client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
    seed_credit_note: uuid.UUID,
) -> None:
    file_id = uuid.uuid4()
    captured: dict[str, Any] = {}
    ...
    r = await api_client.post(
        "/api/v1/attachments",
        data={"entity_kind": "credit_note", "entity_id": str(seed_credit_note)},
        files={"file": ("credit.pdf", b"PDF1", "application/pdf")},
    )
    assert r.status_code == 201, r.text
    assert captured["upload"]["tenant_id"] == DEFAULT_TENANT_ID
    assert captured["link"]["entity_kind"] == "credit_note"
    assert captured["link"]["entity_id"] == seed_credit_note


async def test_upload_cross_tenant_credit_note_returns_403(
    api_client: AsyncClient,
) -> None:
    """A credit note id the tenant doesn't own is rejected 403 by the
    ownership SELECT — without leaking whether the row exists."""
    bogus = uuid.uuid4()
    r = await api_client.post(
        "/api/v1/attachments",
        data={"entity_kind": "credit_note", "entity_id": str(bogus)},
        files={"file": ("credit.pdf", b"PDF1", "application/pdf")},
    )
    assert r.status_code == 403


async def test_list_credit_note_kind_accepted(
    api_client: AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
    seed_credit_note: uuid.UUID,
) -> None:
    """GET with entity_kind=credit_note passes _validate_kind and
    forwards the filter to the vault list call."""
    captured: dict[str, Any] = {}
    ...
    r = await api_client.get(
        "/api/v1/attachments",
        params={"entity_kind": "credit_note", "entity_id": str(seed_credit_note)},
    )
    assert r.status_code == 200, r.text
    assert captured["args"] == ("credit_note", seed_credit_note)
