...
def test_pdf_filename_is_latin1_safe() -> None:
    """Regression (quote b50c6396, 2026-07-03): a title with an em-dash made
    Content-Disposition non-latin-1 and Starlette raised UnicodeEncodeError
    AFTER a successful render — the PDF compiled, the response 500'd."""
    from saebooks.api.v1.quotes import _pdf_filename

    f = _pdf_filename("1025", "Starling — Structural Steel & Metalwork, Aurukun")
    f.encode("latin-1")  # must not raise
    assert f.startswith("SAE-2026-1025-") and f.endswith(".pdf")
    assert "—" not in f
    # degenerate titles fall back cleanly
    assert _pdf_filename("x", None) == "SAE-2026-x-quote.pdf"
    assert _pdf_filename("x", "———").encode("latin-1") == b"SAE-2026-x-quote.pdf"
