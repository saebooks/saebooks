...
def reset_jwt_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    os.environ["SAEBOOKS_SECRET_KEY"] = "test-secret-key-for-signup-tests"
    _reset_secret_cache()
    # Email now routes through the comms module over HTTP (#32). There is no
    # module in the test stack, so stub the facade the signup router bound
    # (``send_raw_email`` imported as ``send_email``) to a no-op — the
    # verification/reset/magic emails are best-effort and swallowed by the
    # router anyway; this keeps the tests offline + deterministic.
    from saebooks.services.email import EmailResult

    async def _fake_send(to, subject, html, **kwargs):
        recipients = [to] if isinstance(to, str) else list(to)
        return EmailResult(mode="outbox", message_id="test", recipients=tuple(recipients))

    monkeypatch.setattr("saebooks.api.v1.signup.send_email", _fake_send)
    yield
    _reset_secret_cache()
