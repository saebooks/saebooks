...
|---|---|
| [`saebooks`](.) | The ledger engine — FastAPI + Postgres. A **pure API service** (REST + MCP + gRPC/Connect), no embedded UI. AGPLv3 + commercial. |
| [`saebooks-web`](../saebooks-web) | Server-rendered frontend — FastAPI + Jinja2 + HTMX, no build step. A thin client over the API, and the only bookkeeper UI. AGPLv3 + commercial. |
...
**Run the community edition** as a self-contained bundle — Postgres +
engine + `saebooks-web` + the LaTeX render service — from
[`deploy/community/`](./deploy/community/) (`docker compose up`; see its
[README](./deploy/community/README.md)).
