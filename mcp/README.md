...
Create `mcp/.env` (mode 600, gitignored) before `docker compose up`:

```
SAEBOOKS_API_TOKEN=saebk_REPLACE_WITH_ISSUED_TOKEN
```

The compose file expands `${SAEBOOKS_API_TOKEN:-}` from this file at startup. Without it the server starts but every tool call 401s.

## Local LAN port

Compose binds `10.0.2.1:18314 → container:8000` on r420. (18313 was already allocated; the README header is the canonical port.)
