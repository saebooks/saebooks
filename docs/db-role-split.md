...
Stacks (one bosun compose project per tenant DB):

| # | Stack | Compose dir | DB container |
|---|---|---|---|
| 1 | sandbox | `/home/sauer/bosun/compose/saebooks-sandbox/` | `saebooks-sandbox-db-1` |
| 2 | sauer (prod) | `/home/sauer/bosun/compose/saebooks-sauer/` | `saebooks-sauer-db-1` |
| 3 | gecairns | `/home/sauer/bosun/compose/saebooks-gecairns/` | `saebooks-gecairns-db-1` |
| 4 | app-preview | `/home/sauer/bosun/compose/saebooks-app-preview/` | `saebooks-app-preview-db-1` |
| 5 | cashbook-demo | `/home/sauer/bosun/compose/saebooks-cashbook-demo/` | `saebooks-cashbook-demo-db-1` |

**Order:** sandbox first, soak 24 h, then sauer, then the other three in any order. The point of sandbox-first is that the worst case for this change is "every request 404s because RLS now actually fires but `SET LOCAL` is missing somewhere"; we want that surfacing on the stack with the lowest cost-of-failure.
...
2. **Wire `SAEBOOKS_APP_DB_PASSWORD` into the API container env.** Edit `/home/sauer/bosun/compose/saebooks-<stack>/docker-compose.yml`, add to the `api` service `environment:` block:
    ...
    Add `SAEBOOKS_APP_DB_PASSWORD=<the password>` to `/home/sauer/bosun/compose/saebooks-<stack>/.env`.
...
3. **Apply the migration.** Restart the API container — its entrypoint runs `alembic upgrade head` which picks up 0128 and uses the env var to set the password.
    ...
    ```bash
    ssh r420 "cd /home/sauer/bosun/compose/saebooks-<stack> && sudo docker compose up -d api"
    ```

4. **Verify.** Two checks on the DB container:

    ```bash
    ssh r420 "sudo docker exec saebooks-<stack>-db-1 \
...
6. **24-h soak on sandbox before promoting** to sauer. Watch `saebooks-sandbox-api-1` logs for sudden 404 spikes on previously-200ing endpoints — that is the SET-LOCAL-missing signature.
...
1. Remove `SAEBOOKS_APP_DATABASE_URL` from `/home/sauer/bosun/compose/saebooks-<stack>/docker-compose.yml`. Leave `SAEBOOKS_APP_DB_PASSWORD` in place — harmless.
2. Restart: `ssh r420 "cd /home/sauer/bosun/compose/saebooks-<stack> && sudo docker compose up -d api"`.
...
```bash
ssh r420 "sudo docker exec saebooks-<stack>-api-1 alembic downgrade 0127_drop_journal_tenant_default"
```
...
- Lane 4 report — `/home/richard/projects/saebooks-critics-reports/audit-trail/2026-05-23-overnight/04-rls-multi-tenant.md`
