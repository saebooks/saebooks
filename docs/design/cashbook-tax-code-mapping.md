...
2. **Capital purchases need a different reporting_type**. BAS G10 is "non-capital purchases (capital)" — i.e. purchases that go to a capital asset account and report under G10 not G11. The current sauer tenant has only `reporting_type IN ('taxable', 'gst_free')`. No `capital` code. `CAP_PURCHASE` cashbook category would land in G11 wrongly.
...
1. **`CAP_PURCHASE` needs a capital-acquisitions tax_code.** Today sauer's CoA doesn't have one. Either:
   - (a) Add `43 GST on capital acquisitions` (rate 10, reporting_type `capital`) to the AU seed and migrate existing tenants to include it. Existing CAP_PURCHASE rows are zero on sauer (cashbook isn't its primary mode) so the migration is low-risk.
