...
`/home/sauer/bosun/compose/saebooks/.env`) is resolved:

1. Set `SAEBOOKS_APP_DATABASE_URL` in
   `/home/sauer/bosun/compose/saebooks/.env`. The password lives at
   `/home/sauer/.claude/secrets/saebooks-app-db.env` (already stashed
   as part of this branch).
2. `ssh r420 'cd /home/sauer/bosun/compose/saebooks && sudo docker compose exec -T api alembic upgrade head'` — applies migration 0084.
