...
We went with option 2. The sidecar adds one container to the bosun
...
`10.0.2.1:18315`) and forwards every request to `api:50051` on the
...
The proxy lives in the bosun compose stack (host-local config — bosun
isn't a git repo). Add this service block to
`bosun/compose/saebooks/docker-compose.yml`:
...
```bash
ssh r420 'cd /home/sauer/bosun/compose/saebooks && sudo docker compose up -d grpcwebproxy'
```
