"""Auth + tenant-context dependencies for the capture module app.

Mirrors ``preaccounting_app.deps`` (#32 step 4) exactly:

1. **Inbound auth** — every ``/module/capture/*`` route is gated by a shared
   secret presented as ``X-Capture-Token`` and compared in constant time
   against ``settings.capture_token`` (env ``CAPTURE_TOKEN``). An EMPTY
   configured token HARD-DISABLES the surface with 503 in every environment:
   this module fronts tenant data, so an unconfigured instance must fail
   closed rather than serve ungated. Same fail-closed contract as
   ``api/internal/auth.py`` and the pre-accounting module.
...
async def require_capture_token(
    ...
    """Gate a module route on the ``X-Capture-Token`` shared secret.
    ...
    """
    expected = settings.capture_token.strip()
    if not expected:
        raise HTTPException(
            status.HTTP_503_SERVICE_UNAVAILABLE,
            "capture module disabled: CAPTURE_TOKEN is not configured",
        )
    presented = (x_capture_token or "").strip()
    if not presented or not secrets.compare_digest(presented, expected):
        raise HTTPException(
            status.HTTP_401_UNAUTHORIZED,
            "invalid or missing X-Capture-Token",
        )
...
async def get_module_session(
    ...
    transaction — including the internal commits the reused engine handlers
    make (wizard commits, statement-line inserts, cursor advances).
    """
    ctx = TenantContext(_parse_tenant_id(request), _parse_company_id(request))
