...
* **Maximum super contribution base** (MSCB) = $62,500 per quarter
  FY25-26 — earnings above this cap don't attract SG. Historically
  AWOTE-indexed; from FY25-26 derived from the concessional
  contributions cap ($30,000 / 12% / 4). History kept in
  ``_HISTORICAL_MSCB`` so back-pay calcs against an older quarter use
  the right cap.
...
_HISTORICAL_MSCB: dict[int, Decimal] = {
    ...
    2025: Decimal("62500"),   # FY25-26
    # ^ From FY25-26 the MSCB is no longer AWOTE-indexed: it is derived
    #   from the concessional contributions cap ($30,000 / 12% SG / 4
    #   quarters = $62,500/qtr) — it went DOWN from FY24-25's $65,070.
    #   From FY26-27 (payday super, 1 Jul 2026) the base becomes an
    #   ANNUAL $270,830 rather than a quarterly cap — a structural
    #   change this quarterly table cannot express; the jurisdiction-
    #   module rework owns it.
...
# Kept equal to the most recent recorded year's value.
_DEFAULT_MSCB = Decimal("62500")
...
def compute_super(
    *,
    ...
    sg_amount = round_money(ote_capped * rate)
