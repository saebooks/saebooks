...
async def _company_default_payment_terms(
    session: AsyncSession, company_id: uuid.UUID
) -> str | None:
    """Company-default free-text payment terms (0171).

    Copied onto the document at CREATE when the caller supplies no
    ``payment_terms`` — a per-document value always wins because the
    default only fires on None.
    """
    company = await session.get(Company, company_id)
    if company is not None and company.default_payment_terms:
        return company.default_payment_terms
    return None
...
async def _replace_lines(
    ...
    *,
    ...
    for i, raw in enumerate(lines, 1):
        await _add_line(session, inv, raw, i, company_id=company_id)
    await session.flush()
...
async def _add_line(
    ...
    *,
    ...
    """Build + add ONE ``InvoiceLine`` from a raw line dict.

    Single source of truth for the line-processing rules (uuid coercion,
    item→income-account forcing, per-company account validation, tax-code
    resolution, margin/retention/trade-in handling, total computation).
    Shared by ``_replace_lines`` (full replace) and ``api_append_line``
    (single append). Adds the line to the session and returns it; the
    caller owns the ``flush``/``_recalc``.
    """
...
async def _validate_source_quote_tenant(
    session: AsyncSession,
    source_quote_id: uuid.UUID,
    tenant_id: uuid.UUID,
) -> None:
    """Raise ``InvoiceError`` if ``source_quote_id`` is not a quote in ``tenant_id``.

    Guards the optional quote back-link on invoice create: a caller-supplied
    ``source_quote_id`` that names a foreign-tenant (or non-existent) quote is
    rejected with a 422 rather than surfacing as a raw FK IntegrityError/500.
    Imported locally to avoid a module-level import cycle (quotes imports
    invoices for its convert-to-invoice hand-off).
    """
    from saebooks.models.quote import Quote

    result = await session.execute(
        select(Quote.id).where(
            Quote.id == source_quote_id,
            Quote.tenant_id == tenant_id,
        )
    )
    if result.scalar_one_or_none() is None:
        raise InvoiceError("source_quote not found in current tenant")
...
async def api_create(
    ...
    *,
    ...
    ``source_quote_id`` optionally stamps the quote→invoice back-link
    (invoices.source_quote_id) on the new invoice, folding what used to be
    a follow-up UPDATE in ``quotes.convert_to_invoice`` into the create.
    When supplied it is validated to name a quote in ``tenant_id``.
    ...
    so a caller (e.g. a two-phase pre-accounting conversion) can commit the
    fact atomically with its own idempotency bookkeeping.
    """
    ...
    if source_quote_id is not None:
        await _validate_source_quote_tenant(session, source_quote_id, tenant_id)
...
async def api_update(
    ...
    *,
    ...
    if inv.status != InvoiceStatus.DRAFT:
        # Non-financial metadata (notes, payment_terms, due_date) may be
        # corrected on POSTED / VOIDED / WRITTEN_OFF invoices — none of it
        # feeds totals, GST or the posted journal entry (due_date appears in
        # no JE; it only drives aging buckets and display, and correcting a
        # wrong due date is exactly the point). Anything financial stays
        # DRAFT-only: void the invoice and raise a new one. Note issue_date
        # and settlement_date stay locked — either can be the JE
        # gl_entry_date, i.e. the GST/BAS period anchor. The change is
...
async def api_append_line(
    ...
    *,
    ...
    """Append ONE line to a DRAFT invoice; return (invoice, created_line).

    Same guards as the PATCH line-replacement path: optimistic locking
    (``expected_version``), DRAFT-only, and cross-tenant FK validation of
    the line's ``account_id`` / ``tax_code_id``. Unlike PATCH — which
    hard-deletes and rebuilds every line (destroying existing line ids and
    thereby breaking any ``time_entries.invoice_line_id`` back-refs) — this
    preserves existing lines and returns the new line so the caller can read
    its freshly-minted id (the time-entries convert-to-invoice-line flow
    writes it back into ``time_entries.invoice_line_id``). Bumps ``version``
    and appends a change_log row like every other edit.
    """
    inv = await _get_with_lines(session, invoice_id)
    if inv is None:
        raise InvoiceError(f"Invoice {invoice_id} not found")
    ...
    if inv.status != InvoiceStatus.DRAFT:
        raise InvoiceError(
            f"invoice_not_draft: cannot append a line to invoice {inv.id} in "
            f"state {inv.status.value}; only DRAFT invoices accept new lines."
        )

    await _validate_line_fks(session, [line], inv.tenant_id)
    ...
    created = await _add_line(
        session, inv, line, int(next_line_no), company_id=inv.company_id
    )
    await session.flush()
    ...
    await change_log_svc.append(
        ...
        actor=actor,
        payload={
            **_serialise_invoice(inv_loaded),
            "line_append": {"line_id": str(created_id), "line_no": int(next_line_no)},
        },
        version=inv_loaded.version,
    )
    if commit:
    ...
    inv_final = await _get_with_lines(session, invoice_id)
    assert inv_final is not None
    created_final = next(
        (ln for ln in inv_final.lines if ln.id == created_id), None
    )
    assert created_final is not None
    return inv_final, created_final
