...
async def gl_movement(
    ...
    *,
    ...
    """Sum posted journal-line movement on a GL account.
    ...
    Optional ``date_from`` / ``date_to`` bound the entry date inclusively
    (``entry_date >= date_from`` and ``entry_date <= date_to``); omit both
    for the all-time total the reconcile sweep uses.

    This is the single source of truth for the movement figure exposed at
    ``GET /api/v1/accounts/{id}/gl-movement`` and consumed by the feed
    reconcile variance — the endpoint and the sweep run the same query.
    """
...
async def _ledger_total(
    ...
    """All-time GL movement on this account — thin wrapper over
    :func:`gl_movement` so the sweep and the fact API share one query."""

    return await gl_movement(session, ledger_account_id)
