...
def _empty_extraction() -> ExtractedStatement:
    """An ExtractedStatement with no fields populated.

    Used when extraction fails so a reviewable header row can still be
    persisted (status NEEDS_REVIEW + extract_error) instead of raising a 5xx.
    """
...
async def ingest_statement(
    ...
    *,
    ...
    #
    # Fail-safe: if the first extraction raises (the vision model rejects a
    # PDF, the LLM call exhausts retries, or the JSON is unparseable) we must
    # NOT bubble a 5xx out of ingest. Instead persist a reviewable header at
    # NEEDS_REVIEW carrying extract_error and return it (#28 defect 6). We
    # keep image_bytes/mime around so the escalation path (#28 defect 5) can
    # re-run vision without a second Paperless download.
    # ------------------------------------------------------------------
    vision_used = False
    ...
    try:
        ...
        logger.warning(
            "extraction failed for doc %d: %s; persisting NEEDS_REVIEW row",
            paperless_document_id,
            exc,
        )
        stmt = await _upsert_header(
            ...
            status=StatementStatus.NEEDS_REVIEW.value,
            extraction_meta={
                **extraction_meta,
        ...
        await session.flush()
        await _upsert_lines(session, stmt, _empty_extraction())
    ...
    if balance_discrepancy > _CENT:
        ...
        try:
            if vision_used:
                # The first extraction came from the vision model on empty/short
                # OCR. Re-running the TEXT extractor on that empty string is
                # useless and can overwrite a good vision parse with a 0-vs-0
                # empty result (#28 defect 5). Re-escalate via vision instead,
                # reusing the already-downloaded image bytes (no second
                # Paperless fetch).
                if image_bytes is not None:
                    escalated_extraction = await extract_statement_vision(
                        image_bytes,
            ...
            if escalated_extraction is None:
                # Nothing to escalate with (vision path with no image bytes).
                ...
                # Guard the swap: never let an escalation that came back empty
                # (no lines OR no supplier_name) replace a populated first
                # extraction (#28 defect 5). A worse parse must not clobber a
                # better one even if its degenerate arithmetic "ties".
                escalation_is_populated = bool(
                    escalated_extraction.lines
                    and escalated_extraction.supplier_name is not None
                )

                if esc_discrepancy <= _CENT and escalation_is_populated:
                    ...
                    extraction_meta["escalation_resolved"] = False
                    extraction_meta["balance_discrepancy"] = str(balance_discrepancy)
                    if esc_discrepancy <= _CENT and not escalation_is_populated:
                        # The escalation "balanced" only because it was empty.
                        extraction_meta["escalation_empty_rejected"] = True
    ...
    #
    # RECONCILED requires BOTH:
    #   (a) the statement's own arithmetic ties (balance_discrepancy <= _CENT)
    #       and no open exceptions (MISSING_IN_BOOKS / AMOUNT_MISMATCH); AND
    #   (b) the books-vs-supplier gap ties: |summary.balance_delta| <= _CENT.
    #
    # Without (b) a statement can look internally clean (every line matched,
    # zero exceptions, internal arithmetic balanced) yet still disagree with
    # our ledger — e.g. a POSTED bill total=1100 amount_paid=1000 ref-matches
    # a 1100 statement line: MATCHED (match compares bill.total to the line),
    # 0 exceptions, balance_discrepancy=0 — but our_ap=100 while the supplier
    # says 1100, so balance_delta=1000. That MUST NOT be stamped RECONCILED
    # (#28 defect 1). When the books gap is the only failure we surface it as
    # NEEDS_REVIEW with a balance_delta_gap note so an operator investigates.
    # ------------------------------------------------------------------
    balance_delta_gap = abs(summary.balance_delta) > _CENT
    ...
    if balance_discrepancy > _CENT:
        ...
        # Open exceptions but internal balance ties
        final_status = StatementStatus.EXTRACTED.value
    elif balance_delta_gap:
        # Internally clean, no open exceptions, but our books disagree with
        # the supplier's closing balance — needs a human.
        final_status = StatementStatus.NEEDS_REVIEW.value
    else:
    ...
    if balance_delta_gap:
        final_meta["balance_delta_gap"] = str(summary.balance_delta)
    stmt.extraction_meta = final_meta
...
async def _resolve_contact(
    ...
    """Exact case-insensitive name match against supplier/contractor/sub-contractor/both contacts."""
    result = await session.execute(
