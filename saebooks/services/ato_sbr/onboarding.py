...
The DB-state helpers below (``get_or_create_config``, ``confirm_step``,
``save_ssid``, ``set_environment``, ``clear_config``, ``status_for``) are
plain wizard bookkeeping and work unchanged in the community edition.
``save_keystore`` and ``test_environment`` delegate to
``services.ato_sbr.keystore.load_keystore`` and
``services.ato_sbr.ping.ping_environment`` respectively — both of which are
community-edition stubs that raise ``NotImplementedError`` (real ATO
Machine Credential parsing + EVTE/production reachability checks are a
commercial SAE Books feature; see CHARTER.md / LICENSING.md). Actual SBR
lodgement (signed SOAP envelopes, token acquisition) is likewise commercial.
"""
