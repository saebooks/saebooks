...
Mirrors the shape of ``services/bank_feeds/errors.py``. Routers map
each subclass to a specific HTTP status (see
``saebooks.api.v1.sync`` for the dispatch table).
"""
...
class SyncUpstreamError(SyncError):
    ...
    Treated as transient — the worker logs and retries on the next
    scheduled poll. Connection status stays ``active``.
    """
...
class SyncValidationError(SyncError):
    ...
    The offending object is quarantined (see plan §8 — failure modes).
    The connection stays ``active`` so other objects continue to sync.
    """
...
class SyncConflictError(SyncError):
    ...
    The worker logs a ``sync_audit_log`` row with ``direction='conflict'``
    and the diff payload. The connection's ``status`` flips to
    ``error`` and the operator must resolve via the Settings -> Sync UI
    (re-push or re-pull). LWW is the default for the unambiguous case
    (only one side moved); this class fires only when both moved.
    """
