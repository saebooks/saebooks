...
* For each row returned, we ``UPSERT`` into our local table keyed on
  ...
  ``uq_<table>_external_id`` (migration 0217) makes this an O(log n)
  lookup.
...
* On success we advance ``connection.last_pulled_at`` to the *server-
  ...
  this makes the watermark robust against clock drift.

* The pull never edits invoice line items on existing posted invoices
  — per ``saebooks-marketing-differentiator`` the rendered
...
async def _upsert_contact(
    ...
    * ``quarantined`` — row was already quarantined; skip.
    ...
    # ``push.py``). We do NOT stamp ``last_pushed_version`` here: the
    # push selector reads ``origin`` for eligibility, so a pulled row
    # correctly stays out of the push candidate set without needing to
    # fake a "already pushed" watermark.
...
async def _upsert_invoice(
    ...
    *,
    ...
    ``saebooks-marketing-differentiator``. New invoices
    ...
    if existing is None:
        ...
        # flow.
        #
        # NOTE (port caveat, not present in the origin branch): a
        # pulled invoice with status=POSTED gets no journal_entry_id —
        # this path bypasses services/journal.py's normal posting flow
        # entirely (by design; there is no Xero-side GL journal to
        # mirror per invoice). Reports that assume every POSTED invoice
        # has a journal_entry_id should treat pulled Xero invoices as
        # an exception. See docs/sync/xero.md.
        existing = Invoice(
            id=uuid.uuid4(),
            tenant_id=connection.tenant_id,
            company_id=company_id,
    ...
    # See ``_upsert_contact`` for why we don't stamp
...
async def _upsert_state(
    ...
    * On UPDATE, ``origin`` is normally NOT supplied (None) — the
      ...
      by the push success-handler. Any other transition would violate
      the state machine, so be explicit about the value.
    """
...
def _merge_contact_type(local: ContactType, pulled: ContactType) -> ContactType:
    ...
    The union of customer + supplier roles is preserved across sync —
    if our side or Xero's side has decided the contact is both, the
    contact stays both.
    """
...
def _merge_invoice_status(
    ...
    Invoice status is monotonic forward — DRAFT -> POSTED -> VOIDED.
    If Xero says DRAFT but we already have POSTED, keep POSTED.
    """
