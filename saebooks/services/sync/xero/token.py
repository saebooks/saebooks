...
used twice in a row (see plan §11.b "Persisted refresh-token rotation").
...
job (the worker's per-connection client factory). We expose a
``on_refresh_rotated`` callback that fires synchronously on every
refresh — the caller writes the new ciphertext under its own
transaction.
"""
...
class XeroTokenCache:
    ...
    (one tenant + one Xero org). The worker's per-connection client
    factory builds a fresh cache for each ``sync_xero(connection)`` call;
    the cache lives only as long as that worker run.
    ...
    most one worker runs per connection at a time (the ``sync_xero``
    entry-point takes a row lock on ``sync_connections`` before doing
    anything).
    """
    ...
    async def _refresh(self) -> _CachedAccessToken:
        ...
        if resp.status_code == 400:
            ...
            # Surface as SyncAuthError so the worker marks the
