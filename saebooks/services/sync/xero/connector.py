...
* The router's ``POST /api/v1/sync/xero/{id}/trigger`` endpoint
  (operator-initiated).
* A future scheduled poll (not yet wired — see docs/sync/xero.md
  "Future work").
...
re-raised to the caller — a future scheduled poll would re-run after
the ``Retry-After`` window.

Tenancy
-------
The session passed in MUST already have ``app.current_tenant`` set to
the connection's tenant — the caller (the trigger router) is
responsible. We guard with a ``current_setting`` check at the top of
the run.
"""
...
async def sync_xero(
    ...
    *,
    ...
    Caller MUST hold a row lock on ``sync_connections`` (the trigger
    router takes ``SELECT ... FOR UPDATE`` before calling this) so two
    runs can't race on the same refresh token.
