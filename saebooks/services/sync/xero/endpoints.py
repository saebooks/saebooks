...
async def list_connections(client: XeroClient) -> list[dict[str, Any]]:
    ...
    raise NotImplementedError(
        "list_connections is implemented inside the OAuth callback "
        "handler in routers/sync.py — it runs *before* an org is "
...
async def post_manual_journals(
    ...
    of truth (per plan §1, pull-direction journals would be noise).
    Push-only.
    """
    body, _headers = await client.post(
        "ManualJournals",
...
async def list_accounts(client: XeroClient) -> list[dict[str, Any]]:
    ...
    Plan §1: read-only on first connect. We never push CoA changes to
    Xero — that's the accountant's domain.
    """
