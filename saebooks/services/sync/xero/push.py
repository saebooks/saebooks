...
async def _select_contact_push_candidates(
    ...
    rows (re-pushing a freshly-pulled row would overwrite its
    ``external_id`` with a fresh POST response and break the upstream
    link).
    ...
    # (migration 0217) is the partial composite that backs this.
    stmt_existing = (
        select(Contact)
        .join(
            SyncState,
