...
2. **429 retry with Retry-After.** Xero returns 429 on rate-limit hits
   ...
   ``Retry-After`` so the worker can sleep until the next scheduled
   poll.
...
class XeroClient:
    ...
    per org. (In practice the worker creates one client per
    ``sync_connections`` row, and each row already maps to a single
    ``external_tenant_id``.)
    """
