...
async def start_wizard(
    ...
    # QBO kind requires Pro+ flag. This edition gate is applied ENGINE-side
    # (it needs the JWT) even when delegating, so the module never has to.
    if payload.kind in _QBO_KINDS:
        await _check_qbo_flag(request)

    # Capture-module delegation (#32 step 5): forward the request verbatim and
    # mirror the module's response. The module owns idempotency + Wizard.start.
    if _capture.delegating():
        return await _cf.mirror_post(
            "imports/wizards",
            await request.body(),
...
async def advance_wizard_step(
    ...
    if _capture.delegating():
        return await _cf.mirror_post(
            f"imports/wizards/{wizard_id}/step",
            await request.body(),
            content_type=request.headers.get("content-type") or "application/json",
            tenant_id=resolve_tenant_id(request),
        )
...
# ---------------------------------------------------------------------------
# GET /imports/wizards — list active wizards for the tenant
# ---------------------------------------------------------------------------
...
async def list_wizards(
    ...
    RLS scopes the result to the calling tenant. Optional ``kind`` filters
    to one import kind. Returns ``{wizards: [...], total: N}``.
    """
    if _capture.delegating():
        params: dict[str, Any] = {"limit": limit}
        if kind is not None:
            params["kind"] = kind
        return await _cf.mirror_get(
            "imports/wizards", params=params, tenant_id=resolve_tenant_id(request)
        )
...
async def get_wizard(
    ...
    if _capture.delegating():
        return await _cf.mirror_get(
            f"imports/wizards/{wizard_id}", tenant_id=resolve_tenant_id(request)
        )
...
async def commit_wizard(
    ...
    # Capture-module delegation (#32 step 5): forward with the engine-resolved
    # active company (X-Company-Id) so the module runs the commit under the
    # same company the JWT path would. The module owns idempotency + dispatch.
    if _capture.delegating():
        return await _cf.mirror_post(
            f"imports/wizards/{wizard_id}/commit",
            await request.body(),
            content_type=request.headers.get("content-type") or "application/json",
            tenant_id=tenant_id,
