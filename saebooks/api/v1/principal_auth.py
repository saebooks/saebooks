...
async def authenticate_begin() -> WebauthnBeginResponse:
    # Platform-module delegation (#32 wave 2): the principal LOGIN half is a
    # public pre-auth ceremony (no principal token yet) that mints an UNBOUND
    # principal token — it moves. begin/finish share a process-local challenge
    # store, so both halves run in the same process. The authenticated half
    # (register / act-as / list-tenants / acting) STAYS engine-side: it is
    # enforcement (require_principal_bearer + resolve_grant_role under
    # FORCE-RLS) — see wave-2 verdict. Flag off → in-process below.
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "principal/auth/webauthn/authenticate/begin", {}
        )
    try:
...
async def authenticate_finish(
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "principal/auth/webauthn/authenticate/finish",
