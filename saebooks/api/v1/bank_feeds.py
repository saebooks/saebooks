...
async def create_connection(
    ...
    # Capture-module delegation (#32 step 5): the feature gate already ran
    # (router dep) before this handler, so the module never re-checks it.
...
async def list_connections(
    ...
    tenant_id = resolve_tenant_id(request)
    if _capture.delegating():
        return await _cf.mirror_get("bank-feeds/connections", tenant_id=tenant_id)
    result = await session.execute(
...
async def get_connection(
    ...
    if _capture.delegating():
        return await _cf.mirror_get(
            f"bank-feeds/connections/{connection_id}", tenant_id=tenant_id
        )
    result = await session.execute(
...
async def delete_connection(
    ...
    tenant_id = resolve_tenant_id(request)
    if _capture.delegating():
        return await _cf.mirror_delete(
            f"bank-feeds/connections/{connection_id}", tenant_id=tenant_id
        )
    result = await session.execute(
        select(BankFeedExternalCred).where(
...
async def sync_transactions(
    ...
    # Capture-module delegation (#32 step 5): the module re-runs the period
    # lock guard + relay sync + persistence against the shared DB.
    if _capture.delegating():
        return await _cf.mirror_post(
            "bank-feeds/sync",
            await request.body(),
