...
class CompanyOut(BaseModel):
    ...
    # Company-default free-text payment terms (0171) — seeds invoice /
    # credit-note payment_terms at CREATE; per-document override wins.
    default_payment_terms: str | None = None
    # Remittance / "How to Pay" details (0168) — previously DB-write-only;
    # exposed here so tenants can configure them through the API. Used as
    # the fallback when no bank account is flagged show_on_invoice.
...
class CompanyUpdate(BaseModel):
    ...
    default_payment_terms: str | None = None
    # Remittance / "How to Pay" details (0168) — API exposure added with 0171.
    bank_name: str | None = None
    bank_bsb: str | None = None
    bank_account_number: str | None = None
    bank_account_name: str | None = None
...
class InvoiceLineAppendOut(BaseModel):
    """Response for ``POST /api/v1/invoices/{id}/lines``.

    Returns the full created line (so the caller can read the new
    ``line.id`` to write back into e.g. ``time_entries.invoice_line_id``)
    plus the invoice id and its post-append version.
    """
...
class InvoiceCreate(InvoiceBase):
    ...
    # Optional back-link to the pre-accounting quote this invoice was
    # minted from (invoices.source_quote_id, SET NULL FK → quotes.id).
    # Stamped on create so the quote→invoice conversion no longer needs a
    # follow-up UPDATE; harmless (stays NULL) for a plain invoice create.
...
class InvoiceOut(BaseModel):
    ...
    # Back-link to the quote this invoice was converted from (nullable).
    source_quote_id: uuid.UUID | None = None
...
class CreditNoteBase(BaseModel):
    ...
    reason: str | None = None
    notes: str | None = None
    # Free-text payment terms (0171). Omitted/None at CREATE = defaulted
    # from Company.default_payment_terms.
    payment_terms: str | None = None
...
class BankAccountCreate(BaseModel):
    ...
    show_on_invoice: bool = Field(
        default=False,
        description=(
            "Render this account's bank details on invoice/credit-note PDFs "
            "(Remit-to). Setting true clears the flag on every other account "
            "in the company."
        ),
    )
...
class BankStatementLineBulkItem(BaseModel):
    """One line in a bulk statement-line create.

    ``external_id`` is REQUIRED when the request carries a
    ``bank_feed_account_id`` (feed provenance — it becomes the upstream
    transaction id deduped by ON CONFLICT); it is ignored for the CSV/OFX
    fingerprint path, which computes its own external_id.
    """
...
class BankStatementLineBulkCreate(BaseModel):
    """POST body for ``/bank_statement_lines/bulk``.

    Dedup strategy is inferred from provenance: when
    ``bank_feed_account_id`` is set, the feed EXTERNAL_ID + ON CONFLICT
    strategy is used (every line must carry ``external_id``); otherwise
    the CSV/OFX FINGERPRINT strategy is used.
    """

    account_id: uuid.UUID
    bank_feed_account_id: uuid.UUID | None = None
    lines: list[BankStatementLineBulkItem]


class BankStatementLineBulkResult(BaseModel):
    """Response for a bulk statement-line create."""
