...
def _pdf_filename(number: str, title: str | None) -> str:
    """ASCII-safe Content-Disposition filename.

    Response headers are latin-1; titles carry em-dashes and other
    non-latin-1 text, which made Starlette raise UnicodeEncodeError AFTER a
    successful render (quote b50c6396, 2026-07-03). Non-ASCII characters are
    dropped, whitespace and quotes become hyphens.
    """
    stem = (title or "quote").encode("ascii", "ignore").decode()
    stem = re.sub(r'[\s"]+', "-", stem).strip("-")[:40] or "quote"
    return f"SAE-2026-{number}-{stem}.pdf"
...
async def get_quote_pdf(
    ...
    filename = _pdf_filename(ctx["number"], ctx["title"])
    return Response(
        content=pdf_bytes,
...
async def post_quote_send_email(
    ...
    pdf_bytes = await render_latex("quote", ctx)
    pdf_filename = _pdf_filename(ctx["number"], ctx["title"])
