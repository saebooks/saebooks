...
selection is future work (see docs/sync/xero.md).
"""
...
# Xero OAuth scopes we request. Read+write on contacts, invoices,
# accounting transactions, and accounts (read-only on CoA).
...
async def _list_xero_connections(
    *,
    ...
    Lives here (not in ``services/sync/xero/endpoints.py``) because at
    this point we have not chosen a tenant yet — ``XeroClient``
    requires a tenant to construct.
    """
...
async def trigger(
    ...
    Returns the run report. This endpoint is the only execution path
    as of this port — a scheduled background worker is future work
    (see docs/sync/xero.md).
    """
    tenant_id = resolve_tenant_id(request)
    stmt = select(SyncConnection).where(
        SyncConnection.id == connection_id,
