...
@router.get("/{po_id}/render-context")
async def get_purchase_order_render_context(
    po_id: UUID,
    request: Request,
    bearer: str = Depends(require_bearer),
    session: AsyncSession = Depends(get_session),
    ...
    """Return the fact context the app render service needs to build the PO PDF.

    Exact ``_build_po_pdf_ctx`` dict fed to the render service by the /pdf route.
    """
    ...
    return JSONResponse({"template": "purchase_order", "kind": "Purchase Order", "ctx": ctx})
