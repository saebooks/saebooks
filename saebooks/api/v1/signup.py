...
from saebooks.services.email import send_raw_email as send_email
...
async def signup(body: SignupRequest) -> MessageResponse:
    ...
    first.
    """
    # Platform-module delegation (#32 wave 1): the rate-limit dependency has
    # already run at the engine edge; forward the validated body to the module,
    # which owns validation + tenant/user mint + launch-promo stamp + the
    # verification email. Flag off (default) → in-process path below runs
    # unchanged.
    if _platform.delegating():
        return await _pf.mirror_post_json("auth/signup", body.model_dump(mode="json"))
...
async def verify_email(body: VerifyEmailRequest) -> TokenResponse:
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/verify-email", body.model_dump(mode="json")
        )
...
async def resend_verification(body: EmailOnlyRequest) -> MessageResponse:
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/resend-verification", body.model_dump(mode="json")
        )
...
async def password_reset_request(body: EmailOnlyRequest) -> MessageResponse:
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/password-reset/request", body.model_dump(mode="json")
        )
...
async def password_reset_confirm(
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/password-reset/confirm", body.model_dump(mode="json")
        )
...
async def magic_link_request(body: EmailOnlyRequest) -> MessageResponse:
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/magic-link/request", body.model_dump(mode="json")
        )
...
async def magic_link_consume(body: TokenOnlyRequest) -> TokenResponse:
    ...
    if _platform.delegating():
        return await _pf.mirror_post_json(
            "auth/magic-link/consume", body.model_dump(mode="json")
        )
