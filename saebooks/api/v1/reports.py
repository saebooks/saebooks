...
# Reporting types that contribute to taxable BAS buckets. Still used
# directly below for the G1/G11 inline aggregation (see _bas_aggregate's
# docstring for why those two are not delegated to tax_return_generator).
_TAXABLE_REPORTING_TYPE = "taxable"

# C3: reuse au.bas_report's exact account-type sets for G1/G11 so this
# endpoint sums over identical lines to the reference engine (only the
...
# M1.5 · T8 — the old hardcoded ``_GST_RATE = Decimal("0.10")`` /
# ``_GST_INCLUSIVE_FRACTION = Decimal(1) / Decimal(11)`` constants that
# used to live here were dead code: nothing in this module ever read
# them — 1A/1B are sourced from the GST control-account balances (see
# ``_bas_gst_amounts``'s "Round-2 audit fix #6" docstring below), never
# from a rate multiplication. The RefTaxCode-sourced replacement is
# ``tax_return_generator.indirect_tax_inclusive_fraction(jurisdiction)``
# (generic ``rate / (100 + rate)``, e.g. AU GST 10 -> 1/11) — available
# for any future caller that legitimately needs an inclusive-fraction
# estimate, but deliberately not wired into 1A/1B here, for the same
# reason the dead constants never were.
...
async def _bas_aggregate(
    ...
    M1.5 · T8 — G2/G3/G10 are read via
    ...
    reconciliation this module already guaranteed for G2/G10).

    G1 and G11 stay inline here, NOT delegated, because this endpoint's
    G1/G11 are intentionally GST-EXCLUSIVE (net sales/purchases only),
    while the box definitions (and au.bas_report, the AU tax engine's
    reference implementation) report G1/G11 GST-INCLUSIVE. That
    divergence is pinned by two passing tests that predate this theme:

      * ``tests/services/test_cashbook_bas.py::
        test_keystone_cashbook_gst_sale_flows_to_bas_g1`` — a $1,100
        GST-inclusive cashbook sale must move au.bas_report's G1 by
        exactly 1100 (inclusive).
      * this module's own ``test_bas_mid_quarter_registration_split`` —
        1A must equal ``g1_post_registration * 10%``, which only holds
        if this endpoint's g1_post is GST-EXCLUSIVE (a $3000 net sale
        with $300 GST: 300 == 3000 * 0.10, but NOT 300 == 3300 * 0.10).

    Routing G1/G11 through the shared inclusive box definitions would
    silently change this endpoint's numbers and break both — so they
    stay bespoke by design here, computed against the exact same
    tenant/status/archived/date-range scope as the delegated boxes.
    """
    from saebooks.services.tax_return_generator import aggregate_return_boxes

    boxes = await aggregate_return_boxes(
        session,
        company_id,
        jurisdiction="AU",
        return_type="BAS",
        ...
        to_date=to_date,
        tenant_id=tenant_id,
        statuses=reports_svc.REPORTABLE_STATUSES,
        exclude_archived=True,
    )
    g2 = boxes.get("G2", Decimal("0"))
    g3 = boxes.get("G3", Decimal("0"))
    g10 = boxes.get("G10", Decimal("0"))
    ...
    for acc_type, reporting_type, total_debit, total_credit in rows:
        ...
        if acc_type in _AU_BAS_INCOME_TYPES and rt == _TAXABLE_REPORTING_TYPE:
            g1 += total_credit - total_debit
        if acc_type in _AU_BAS_PURCHASE_TYPES and rt == _TAXABLE_REPORTING_TYPE:
            g11 += total_debit - total_credit
