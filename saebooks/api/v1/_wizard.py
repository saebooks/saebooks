...
class Wizard:
    ...
    async def list_active(
        ...
        *,
        ...
        """Return the current tenant's non-expired wizards, newest first.

        RLS scopes this to the calling tenant automatically (the
        ``get_session`` dep binds ``app.current_tenant``), so no explicit
        tenant filter is needed. Optional ``kind`` narrows to one wizard
        kind (e.g. ``"bank_csv"``).

        Each item is ``{wizard_id, kind, step, state, expires_at}`` — the
        shape the ``list_imports`` MCP tool advertises so a calling model
        can resume or commit an in-flight import.
        """
        import json as _json
        ...
        if kind:
            sql += " AND kind = :kind"
            params["kind"] = kind
        sql += " ORDER BY expires_at DESC LIMIT :limit"

        rows = (await session.execute(text(sql).bindparams(**params))).all()
        out: list[dict[str, Any]] = []
        for row in rows:
            raw_state = row.state
            state = _json.loads(raw_state) if isinstance(raw_state, str) else dict(raw_state)
            out.append(
                {
                    "wizard_id": str(row.id),
                    "kind": row.kind,
