...
from saebooks.services.email import send_raw_email
...
async def stripe_webhook(
    ...
    # Platform-module delegation (#32 wave 1): forward the RAW body (the Stripe
    # signature is computed over the exact bytes) plus the Stripe-Signature
    # header to the module, which re-verifies the signature and mints/updates
    # the tenant in-process. Flag off (default) → the in-process path below
    # runs unchanged. The module owns the missing-signature / bad-signature /
    # not-configured responses, so they are mirrored back identically.
    if _platform.delegating():
        fwd = {"Stripe-Signature": stripe_signature} if stripe_signature else None
        return await _pf.mirror_post_raw(
            "billing/webhook",
            payload,
