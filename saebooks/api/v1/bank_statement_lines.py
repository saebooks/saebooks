...
# ---------------------------------------------------------------------------
# Bulk create (fact API — imports + bank feeds ingest through this)
# ---------------------------------------------------------------------------


@router.post("/bulk", response_model=BankStatementLineBulkResult, status_code=201)
async def bulk_create_bank_statement_lines(
    request: Request,
    ...
    """Bulk-create statement lines with dedup — the capture fact API.

    Runs the exact same shared implementation the in-process writers use
    (``services.statement_lines_bulk``), so semantics can't drift:

    * No ``bank_feed_account_id`` → FINGERPRINT dedup (CSV / OFX): a
      deterministic hash + intra-batch occurrence index. Re-posting the
      same file returns ``created=0``; genuinely-distinct rows that share a
      base fingerprint all survive.
    * ``bank_feed_account_id`` set → EXTERNAL_ID dedup (aggregator feed):
      ON CONFLICT DO NOTHING on ``(bank_feed_account_id, external_id)``.
      Every line MUST carry ``external_id``.

    Returns ``{created, deduped, ids}`` (``created + deduped == len(lines)``).
    Same bearer auth + tenant scoping as the single-row POST.
    """
    tenant_id = resolve_tenant_id(request)

    if payload.bank_feed_account_id is not None:
        strategy = DedupStrategy.EXTERNAL_ID
        for idx, ln in enumerate(payload.lines):
            if not ln.external_id:
                raise HTTPException(
                    422,
                    f"line[{idx}] missing external_id — required when "
                    "bank_feed_account_id is provided (feed provenance)",
                )
    else:
