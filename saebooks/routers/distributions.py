...
async def _all_accounts(company_id: uuid.UUID) -> list[Account]:
...
async def distributions_list(request: Request) -> HTMLResponse:
...
async def distributions_new_form(request: Request) -> HTMLResponse:
    company = await _first_company()
    accounts = await _all_accounts(company.id)
...
async def distributions_create(request: Request) -> RedirectResponse | HTMLResponse:
    company = await _first_company()
...
async def distributions_detail(
    request: Request, distribution_id: uuid.UUID
) -> HTMLResponse:
    company = await _first_company()
    async with AsyncSessionLocal() as session:
...
async def distributions_delete(distribution_id: uuid.UUID) -> RedirectResponse:
    async with AsyncSessionLocal() as session:
        try:
