...
class TaxCode(CompanyScoped, Base):
    ...
    # M1.5 · T1 — canonical tax family + credit consequence, alongside the
    # legacy free-text ``tax_system`` (kept for back-compat; ``tax_family`` is
    # the canonical field). Additive/defaulted: existing rows → vat_gst.
    tax_family: Mapped[str] = mapped_column(
        String(16), nullable=False, default="vat_gst", server_default="vat_gst"
    )
