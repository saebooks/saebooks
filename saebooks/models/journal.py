...
class JournalOrigin(enum.StrEnum):
    ...
    # Dutiable transaction event (M1.5 · T5) — a posted stamp/transfer/
    # conveyance/securities/insurance duty assessment. Stored as a plain
    # string on the String(32) origin column — NO DB migration needed
    # (same posture as the two BAD_DEBT_* values above).
    DUTY = "DUTY"
...
class JournalLine(Base):
    ...
    # M1.5 · T2 — normalised 1:many tax components (co-existing taxes on one
    # line). Read-only via this relationship (lazy="raise" — load explicitly
    # with selectinload/a query); WRITES go in directly by FK in
    # services.journal._apply_tax_treatment (async can't append to an unloaded
    # collection). passive_deletes relies on the DB ON DELETE CASCADE rather
    # than loading children to delete them.
    tax_components: Mapped[list["JournalLineTaxComponent"]] = relationship(
