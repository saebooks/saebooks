...
RLS plumbing (``sync-feeds``, ``refresh-feed-issues``, ``inbox-poll-mail``,
``inbox-extract``)
------------------------------------------------------
The cross-tenant CLI walkers run as the non-BYPASSRLS ``saebooks_app``
role and set ``app.current_tenant`` per group via ``SET LOCAL`` inside
a transaction. They refuse to start if the connecting role can bypass
RLS — passing ``--allow-bypass`` is the explicit override (used in dev
or a one-off owner-role rescue).

Cross-tenant enumeration goes through SECURITY DEFINER functions —
``bank_feeds_active_accounts_for_sync()`` (migration 0084),
``inbox_email_addresses_for_poll()`` and
``inbox_documents_tenants_for_sweep()`` (migration 0176) — so the
...
# --------------------------------------------------------------------------- #
# inbox-poll-mail / inbox-extract (Document Inbox phase 3, issue #33)          #
# --------------------------------------------------------------------------- #


async def _inbox_poll_mail(*, allow_bypass: bool) -> int:
    """Drain the email-in catch-all mailbox once (spec §4).

    Cross-tenant walker: routing comes from the SECURITY DEFINER
    enumerator, each message's writes run under
    ``SET LOCAL app.current_tenant``. Per-message errors are logged and
    leave the message in the mailbox for the next cron fire.
    """
    from saebooks.services import inbox_mail
    from saebooks.services.features import FLAG_INBOX_EMAIL, is_enabled

    if not is_enabled(FLAG_INBOX_EMAIL):
        logger.error(
            "inbox-poll-mail: FLAG_INBOX_EMAIL is not enabled on this "
            "edition (%s) — email-in is Business and up.",
            settings.edition,
        )
        return 2
    try:
        source = inbox_mail.mail_source_from_settings(settings)
    except inbox_mail.MailNotConfiguredError as exc:
        logger.error("inbox-poll-mail: %s", exc)
        return 2
        ...
        if not allow_bypass:
            async with SessionFactory() as session:
                try:
                    ...
                    logger.error("inbox-poll-mail: %s", exc)
                    return 2
        outcome = await inbox_mail.poll_mailbox(
            source, SessionFactory, settings=settings
        )
    finally:
        await source.close()
    ...
    logger.info(
        ...
        "documents=%d duplicates=%d skipped=%d",
...
async def _enumerate_sweep_tenants(session: AsyncSession) -> list[uuid.UUID]:
    """Tenants with claimable/reclaimable inbox documents, via the
    SECURITY DEFINER enumerator (migration 0176)."""
    rows = (
        await session.execute(
...
async def _inbox_extract(*, allow_bypass: bool, batch: int) -> int:
    """The extraction sweep (spec §5) — the guarantee behind the
    interactive path.

    Per tenant discovered by the SECDEF enumerator: reclaim EXTRACTING
    rows with claims older than 10 minutes, claim up to ``batch`` due
    RECEIVED rows (FOR UPDATE SKIP LOCKED), extract each. The tenant GUC
    is re-bound on every transaction via ``session.info['tenant_id']``
    and the process-wide ``after_begin`` listener (the API's own
    mechanism) because the service functions commit per document.
    """
    # Importing deps registers the after_begin GUC listener.
    from saebooks.api.v1 import deps as _deps  # noqa: F401
    ...
    if not is_enabled(FLAG_DOCUMENT_INBOX):
        logger.error(
            "inbox-extract: FLAG_DOCUMENT_INBOX is not enabled on this "
            "edition (%s).",
            settings.edition,
        )
        return 2
    ...
    try:
        ...
        logger.error("inbox-extract: %s", exc)
        return 2
    ...
    async with SessionFactory() as enum_session:
        if not allow_bypass:
            try:
                ...
                logger.error("inbox-extract: %s", exc)
                return 2
        tenants = await _enumerate_sweep_tenants(enum_session)

    if not tenants:
        logger.info("inbox-extract: nothing to sweep")
        return 0

    # Below Business the model is never consulted — claimed documents
    # land NEEDS_REVIEW empty for manual keying (same as the upload path).
    extract_enabled = is_enabled(FLAG_AI_EXTRACTION)

    processed = reclaimed_total = 0
    failed_tenants = 0
    for tenant_id in tenants:
        try:
            async with SessionFactory() as session:
                session.info["tenant_id"] = str(tenant_id)
                reclaimed_total += await inbox_svc.sweep_reclaim(
                    session, tenant_id
                )
                claimed = await inbox_svc.sweep_claim(
                    session, tenant_id, batch=batch
                )
                logger.info(
                    "inbox-extract: tenant=%s claimed=%d", tenant_id, len(claimed)
                )
            ...
            failed_tenants += 1
            logger.exception(
                "inbox-extract: tenant=%s failed: %s", tenant_id, exc
            )

    logger.info(
        "inbox-extract: %d tenant(s), reclaimed=%d processed=%d failed_tenants=%d",
        len(tenants),
        reclaimed_total,
        processed,
        failed_tenants,
...
def main(argv: list[str] | None = None) -> int:
    ...
    poll_mail = sub.add_parser(
        "inbox-poll-mail",
        help="Drain the Document Inbox email-in catch-all mailbox once "
        "(token routing, quarantine on miss, attachments-first ingest).",
    )
    poll_mail.add_argument(
        "--allow-bypass",
        action="store_true",
        default=False,
        help="Override the BYPASSRLS-role refusal. NOT for production use.",
    ...
    inbox_ext = sub.add_parser(
        "inbox-extract",
        help="Sweep the Document Inbox extraction queue: reclaim stale "
        "claims, claim due RECEIVED documents (SKIP LOCKED), extract.",
    )
    inbox_ext.add_argument(
        "--allow-bypass",
        action="store_true",
        default=False,
        help="Override the BYPASSRLS-role refusal. NOT for production use.",
    ...
    if args.command == "inbox-poll-mail":
        return asyncio.run(_inbox_poll_mail(allow_bypass=args.allow_bypass))
    if args.command == "inbox-extract":
        return asyncio.run(
