...
# ``/internal/`` is module-to-engine JSON (numbering, demo provision), never
# an HTML page: skipping it also keeps this middleware from binding the
# request-scoped active-company scope-guard contextvar, which would otherwise
# inject a spurious ``company_id = <seed company>`` filter into the
# CompanyScoped queries those endpoints run against an explicit company_id.
_SKIP_PREFIXES = (
    "/static/",
    "/api/",
    "/internal/",
